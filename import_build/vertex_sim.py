...
# H3
iniCond = [
    State(np.array([1.,np.pi/2.,0]),np.array([-1,0,0])),
    State(np.array([1.,np.pi/2.,np.pi/2.]),np.array([-1,0,0]))
]
...
frames=50
def animate(i):
    ax1.plot3D(part1plot[:int(tmaxNum*i/frames),0],part1plot[:int(tmaxNum*i/frames),1],part1plot[:int(tmaxNum*i/frames),2], label="particle 1")
    ax1.plot3D(part2plot[:int(tmaxNum*i/frames),0],part2plot[:int(tmaxNum*i/frames),1],part2plot[:int(tmaxNum*i/frames),2], label="particle 2")
    ax1.legend(loc= 'lower left')
...
anim.save('./h3test.mov', writer='imagemagick')