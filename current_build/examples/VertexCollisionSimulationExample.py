...
from src.test_system_simulations.test_system_bank import dynfunc_h3sim2ballcol, dynjac_h3sim2ballcol, dynfunc_s3sim2ballcol, dynjac_s3sim2ballcol
from src.test_system_simulations.VertexCollisionSimulation import VertexCollisionSimulation

### Example simulation of a pair of free vertices in 3-dimensional hyperbolic space (H3)
...
v = .5              # Initial Velocity (perpendicular to connecting geodesic between vertices)
m1 = 1.             # Mass of vertex 1
m2 = 1.             # Mass of vertex 2
...
startvec = np.array([
    [2.,np.pi/2.,2.*np.pi/4.],                      # Initial Position of vertex 1
    [2.,np.pi/2.,0.*np.pi/4.],                      # Initial Position of vertex 2
    killingvech3([2.,np.pi/2.,2.*np.pi/4.],-v,"y"), # Initial Velocity of vertex 1
    killingvech3([2.,np.pi/2.,0.*np.pi/4.],-v,"x")  # Initial Velocity of vertex 2
    ]).flatten()

## Collision tolerance
tol = 1e-8      # Tolerance to determine vertex collision

## Solver 
solver_id = "gs1"   # Here using Gauss collocation method with 1 internal step

## Initialize Simulation Object and Run Simulation
sim_test = VertexCollisionSimulation(geometry, params, dt, t_max, solver_id)
...
data1 = np.load("h3_c_{}_sim_tmax{}_dt{}.npy".format(solver_id, str(t_max), str(dt)))
